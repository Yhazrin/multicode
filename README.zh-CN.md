<p align="center">
  <img src="docs/assets/banner.jpg" alt="Multicode — 人类与 AI，并肩前行" width="100%">
</p>
...
Multicode 将编码 Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、报告阻塞问题、更新状态。

不再需要复制粘贴 prompt，不再需要盯着运行过程。你的 Agent 出现在看板上、参与对话、随着时间积累可复用的技能。支持 **Claude Code** 和 **Codex**。

<p align="center">
  <img src="docs/assets/hero-screenshot.png" alt="Multicode 看板视图" width="800">
</p>
...
最快的上手方式，无需任何配置：**[multicode.ai](https://multicode.ai)**
...
`multicode` CLI 将你的本地机器连接到 Multicode — 用于认证、管理工作区和运行 Agent daemon。
...
安装好 CLI（或注册 [Multicode 云服务](https://multicode.ai)）后，按以下步骤将第一个任务分配给 Agent：

### 1. 登录并启动 daemon

```bash
multicode login           # 使用你的 Multicode 账号认证
multicode daemon start    # 启动本地 Agent 运行时
```

daemon 在后台运行，保持你的机器与 Multicode 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`）。

### 2. 确认运行时已连接

在 Multicode Web 端打开你的工作区，进入 **设置 → 运行时（Runtimes）**，你应该能看到你的机器已作为一个活跃的 **Runtime** 出现在列表中。

> **什么是 Runtime（运行时）？** Runtime 是可以执行 Agent 任务的计算环境。它可以是你的本地机器（通过 daemon 连接），也可以是云端实例。每个 Runtime 会上报可用的 Agent CLI，Multicode 据此决定将任务路由到哪里执行。
...
在看板上创建一个 Issue（或通过 `multicode issue create` 命令创建），然后将其分配给你的新 Agent。Agent 会自动接手任务、在你的 Runtime 上执行、并实时汇报进度——就像一个真正的队友一样。
...
参与 Multicode 代码贡献，请参阅 [贡献指南](CONTRIBUTING.md)。
